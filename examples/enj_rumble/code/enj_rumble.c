...
static char str_buffer[STRBUFSIZE];
...
void render(void* __unused) {
    ...
    int twidth = enj_font_string_width("Rumble Accessory Tester",
                                       enj_qfont_get_header());
    ...
    enj_qfont_write("effect hex value:", textpos_x, textpos_y,
                    PVR_LIST_PT_POLY);
    ...
    textpos_x = 10;
    minifont_set_color(255, 255, 255); /* White */
    enj_qfont_write("Field description:", textpos_x, textpos_y,
                    PVR_LIST_PT_POLY);
    minifont_set_color(255, 0, 0); /* RED */
    textpos_x = 170;
    textpos_x += enj_qfont_write(" [", textpos_x, textpos_y, PVR_LIST_PT_POLY);
    textpos_x += enj_qfont_write(fieldnames[cursor_pos], textpos_x, textpos_y,
                                 PVR_LIST_PT_POLY);
    textpos_x += enj_qfont_write("]", textpos_x, textpos_y, PVR_LIST_PT_POLY);
    textpos_y += enj_qfont_get_header()->line_height;
    textpos_x += 20;
    ...
    enj_qfont_write(field_descriptions[cursor_pos * 2], textpos_x, textpos_y,
                    PVR_LIST_PT_POLY);
    textpos_y += enj_qfont_get_header()->line_height;
    enj_qfont_write(field_descriptions[cursor_pos * 2 + 1], textpos_x,
                    textpos_y, PVR_LIST_PT_POLY);
    if (loaded_pattern >= 0) {
        textpos_y = 200;
        textpos_x = 10;
        ...
        textpos_y += enj_qfont_get_header()->line_height;
        enj_qfont_write(catalog[loaded_pattern].description, textpos_x,
                        textpos_y, PVR_LIST_PT_POLY);
        textpos_y += enj_qfont_get_header()->line_height;
    }
    ...
    for (size_t i = 0; i < sizeof(instructions) / sizeof(instructions[0]);
         i++) {
        enj_qfont_write(instructions[i], textpos_x, textpos_y,
                        PVR_LIST_PT_POLY);
        textpos_y += enj_qfont_get_header()->line_height;
    }
}
