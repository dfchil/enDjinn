...
void render(void* data) {
    ...
    // enj_draw_sprite(corners, &static_dr_state, &font_hdr, NULL);


    pvr_sprite_hdr_t* font_hdr_copy =
    ...
    *font_hdr_copy = font_hdr;
    ...
    for (char c = 32; c <= 126; c++) {
        fontstartx += 
            enj_font_render_glyph(c, &font, fontstartx, fontstarty, 1.0f, &static_dr_state);
        if (fontstartx > vid_mode->width - 40) {
            fontstartx = 20 * ENJ_XSCALE;
            fontstarty += font.line_height;
        }
    }
    char* li = lorum_ipsum;
    fontstartx = 20 * ENJ_XSCALE;
    fontstarty += font.line_height << 1;
    while(*li != '\0') {
        fontstartx +=
            enj_font_render_glyph(*li, &font, fontstartx, fontstarty, 1.0f, &static_dr_state);
        if (fontstartx > vid_mode->width - 40) {
            fontstartx = 20 * ENJ_XSCALE;
            fontstarty += font.line_height;
        }
...
void setup_font() {
    ...
    if (!enj_font_TR_header(&font, &font_hdr, 1,
    ...
    // Now font_hdr can be used to render text with
    // enj_font_render_text_in_box()
}
