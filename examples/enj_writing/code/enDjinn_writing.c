...
void render(void *data) {
  ...
  *font_hdr_sq = *cur_font_pvr_hdr;
  ...
  for (char c = ' '; c <= '~'; c++) {
    fontstartx += enj_font_render_glyph(c, cur_font_hdr, fontstartx, fontstarty,
  ...
  *font_hdr_sq = *cur_font_pvr_hdr;
  ...
  for (char c = ' '; c <= '~'; c++) {
      fontstartx +=
          enj_font_render_glyph(c, &cmunrm_font_hdr, fontstartx,
          fontstarty, 2.0f, &static_dr_state);
      if (fontstartx > vid_mode->width - 40) {
          fontstartx = 20 * ENJ_XSCALE;
          fontstarty += cmunrm_font_hdr.line_height;
      }
  }
  ...
  while (*li != '\0') {
    fontstartx += 1 + enj_font_render_glyph(*li, cur_font_hdr, fontstartx,
                                            fontstarty, 2.0f, &static_dr_state);
    if (fontstartx > vid_mode->width - 40) {
      fontstartx = 20 * ENJ_XSCALE;
      fontstarty += cur_font_hdr->line_height;
...
void setup_fonts() {
  ...
  if (!enj_font_TR_header(&cmunrm_font_hdr, &cmunrm_font_pvr_hdr, 2,
