...
int enj_font_PAL_OP_header(enj_font_header_t* font, pvr_sprite_hdr_t* hdr,
    ...
    pvr_sprite_cxt_t cxt;
    pvr_sprite_cxt_txr(
        &cxt, PVR_LIST_OP_POLY, PVR_TXRFMT_PAL4BPP | (palette_entry << 25),
...
int enj_font_PAL_PT_header(enj_font_header_t* font, pvr_sprite_hdr_t* hdr,
    ...
    palette_color_mixer(front_color, back_color, palette_entry, pal_fmt);
    pvr_set_pal_entry(palette_entry << (pal_fmt == PVR_PAL_ARGB8888 ? 8 : 4),
                      0);

    // setup header
    pvr_sprite_cxt_t cxt;
    pvr_sprite_cxt_txr(
        &cxt, PVR_LIST_PT_POLY, PVR_TXRFMT_PAL4BPP | (palette_entry << 25),
        1 << font->log2width, 1 << font->log2height,
        (pvr_ptr_t)(uintptr_t)font->pvr_data, PVR_FILTER_NEAREST);
    pvr_sprite_compile(hdr, &cxt);
...
int enj_font_glyph_uv_coords(enj_font_header_t* font, char glyph, uint32_t* auv,
    ...
    *auv = PVR_PACK_16BIT_UV(
        startx * inv_txr_width,
        (float)(font->line_height * (glyph_start.line + 1)) * inv_txr_height);

    *buv = PVR_PACK_16BIT_UV(
        startx * inv_txr_width,
        (float)(font->line_height * glyph_start.line) * inv_txr_height);
    *cuv = PVR_PACK_16BIT_UV(
        (float)(glyph_end.x_min) * inv_txr_width,
        (float)(font->line_height * glyph_start.line) * inv_txr_height);
...
int enj_font_render_text(const char* text, enj_font_header_t* font, uint16_t x,
    ...
    while (*text != '\0') {
        x_pos +=
            enj_font_render_glyph(*text, font, x_pos, y, zvalue, state_ptr);
    }
    return x_pos - x;
}
...
pvr_ptr_t enj_font2texture(enj_font_header_t* font, uint8_t* data_4bpp,
                           pvr_pixel_mode_t mode, enj_color_t front_color,
                           enj_color_t back_color) {
