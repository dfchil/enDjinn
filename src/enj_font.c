...
int enj_font_render_text(const char *text, enj_font_header_t *font, uint16_t x,
  ...
  while (*text != '\0') {
    x_pos += enj_font_render_glyph(*text, font, x_pos, y, state_ptr);
  }
  return x_pos - x;
}
...
pvr_ptr_t enj_font2texture(enj_font_header_t *font, uint8_t *data_4bpp,
  ...
  case PVR_PIXEL_MODE_ARGB4444:
    for (int i = 0; i < width * height; i++) {
      buffer[i] =
          0xffffff00| extr_4bpp_pixel(data_4bpp, i);
    }
    break;
  case PVR_PIXEL_MODE_ARGB1555:
    enj_color_t diff = {.a = (front_color.a - back_color.a) / 15,
                          .r = (front_color.r - back_color.r) / 15,
                          .g = (front_color.g - back_color.g) / 15,
                          .b = (front_color.b - back_color.b) / 15};
    ...
    for (int i = 0; i < width * height; i++) {
      ...
      buffer[i] = (pixel_4bpp > 0)<<15 | ((back_color.r + diff.r * pixel_4bpp) & 0x1F) << 10 |
                 ((back_color.g + diff.g * pixel_4bpp) & 0x1F) << 5 | ((back_color.b + diff.b * pixel_4bpp) & 0x1F);
    }
    break;
