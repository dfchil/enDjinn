...
int enj_texture_load_blob(const void* data, enj_texture_info_t* texinfo) {
    ...
    texinfo->flags.compressed = fDtIsCompressed((fDtHeader*)&texinfo->hdr);
    texinfo->flags.mipmapped = fDtIsMipmapped((fDtHeader*)&texinfo->hdr);
    texinfo->flags.palettised = fDtIsPalettized((fDtHeader*)&texinfo->hdr);
    texinfo->flags.num_palette_colors = fDtGetColorsUsed((fDtHeader*)&texinfo->hdr);
    ...
    texinfo->flags.strided = fDtIsStrided((fDtHeader*)&texinfo->hdr);
    texinfo->flags.twiddled = fDtIsTwiddled((fDtHeader*)&texinfo->hdr);
...
int enj_texture_bind_palette(enj_texture_info_t* texinfo,
                             size_t palette_offset) {
    if (!texinfo->flags.palettised) {
        ENJ_DEBUG_PRINT("Error: texture is not palettised!\n");
        return 0;
    }
    size_t pal_num = texinfo->flags.palette_format == PVR_PAL_ARGB4444
                         ? palette_offset >> 4
                         : palette_offset >> 8;

    texinfo->pvrformat |=
        (pal_num << (texinfo->flags.palette_format == PVR_PAL_ARGB4444 ? 21
                                                                       : 25));
