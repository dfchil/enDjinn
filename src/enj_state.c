...
void enj_state_set_exit_pattern(uint32_t pattern) {
...
void enj_run(void) {
  ...
  while (1) {
    ...
    if (enj_mode_get_index() != state->title_screen_mode_stack_index) {
      enj_ctrlr_state_t **cstates = enj_get_ctrlr_states();
      for (int i = 0; i < MAPLE_PORT_COUNT; i++) {
        if (cstates[i] && enj_ctrlr_button_combo_raw(cstates[i]->buttons.raw,
                                                     state->exit_pattern)) {
