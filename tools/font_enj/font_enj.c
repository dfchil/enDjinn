...
static const int num_glyphs = '~' - ' '; // from '!' to '~'
typedef struct {
  char glyph;
  struct {
    int start;
...
int calculate_sheet_sizes(glyph_rendering_t renderings[num_glyphs],
  ...
  while (!found_sizes && sheet_width < 2048 && sheet_height < 2048) {
    ...
    char glyph = '!';
    for (; glyph <= '~'; ++glyph) {
      int glyph_index = glyph - '!';
      if (x + renderings[glyph_index].width > sheet_width) {
        x = 0;
        ++line;
      }
      int byteOffset = x + (line * line_height * sheet_width);
      ...
      /* advance x */
      if (glyph > '!') {
        renderings[glyph_index - 1].offset_info->line = (uint16_t)line;
        renderings[glyph_index - 1].offset_info->offset_end = (uint16_t)(x);
      }
      x += renderings[glyph_index].width;
    }
    if (glyph >= '~') {
      found_sizes = 1;
      if (verbose_flag) {
...
int generate_glyph(glyph_rendering_t *rendering, stbtt_fontinfo *info,
  ...
  rendering->offset.start = 0;
  rendering->offset.end = ext_buffer_width;
  ...
  for (int xp = int_buf_offset_x; xp < ext_buffer_width; xp++) {
    for (int yp = int_buf_offset_y; yp < int_buf_offset_y + line_height; yp++) {
      if (local_buffer[yp * internal_buffer_width + xp] != 0) {
        rendering->offset.start = xp - int_buf_offset_x;
  ...
  for (int xp = ext_buffer_width + int_buf_offset_x; xp >= int_buf_offset_x;
    ...
    for (int yp = int_buf_offset_y; yp < int_buf_offset_y + line_height; yp++) {
      if (local_buffer[yp * internal_buffer_width + xp] != 0) {
        rendering->offset.end = xp + 1 - int_buf_offset_x;
  ...
  for (int yp = 0; yp < line_height; yp++) {
    for (int xp = rendering->offset.start; xp < rendering->offset.end; xp++) {
  ...
  rendering->width = rendering->offset.end - rendering->offset.start;
}
...
int font_genenerator(int line_height, char *font_path, char *output_path,
  ...
  // ascent = roundf(ascent * scale);
  // descent = roundf(descent * scale);
  glyph_rendering_t renderings[num_glyphs];
  uint8_t *render_buffer = calloc(num_glyphs, line_height * line_height * 2);
  for (int i = 0; i < num_glyphs; ++i) {
    renderings[i].glyph = (char)(i + '!');
    renderings[i].data = render_buffer + (i * line_height * line_height * 2);
    renderings[i].offset_info = &header.glyph_starts[i];
  ...
  for (char glyph = 0; glyph < num_glyphs; ++glyph) {
    ...
    int y = 0;
    if (glyph > 0) {
      x = header.glyph_starts[glyph - 1].offset_end;
      y = header.glyph_starts[glyph - 1].line * line_height;
    }
    printf("placing glyph '%c' at %d,%d (width: %d)\n", renderings[glyph].glyph,
           x, y, renderings[glyph].width);

    for (int yp = 0; yp < line_height; yp++) {
      for (int xp = 0; xp < renderings[glyph].width; xp++) {
        uint8_t v1 =
            renderings[glyph].data[yp * line_height * 2 +
                                   (xp + renderings[glyph].offset.start)];
        bitmap[(y + yp) * sheet_width + (x + xp)] = v1;
      }
    }
  }
